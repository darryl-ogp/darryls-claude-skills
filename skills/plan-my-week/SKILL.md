...
- A DS meeting's prep is a single dedicated "Prep for DS \<name\>" block (see "Prepping for DS meetings" below) — recognize any action about presenting a case, brief, or feedback to that same DS as already covered by it once it exists, however the action happens to be worded (e.g. "prepare and present the Workable approval case to DS" *is* "Prep for DS Jamie" if that's the meeting it's for). Don't create a second block for the same DS meeting under the action's own wording.
- Any other 1:1 or meeting already on the calendar likely already carries its own prep — don't add a separate "prep for X" focus block for it unless the action clearly needs more than the standard 30-min buffer.
...
- A quick administrative/config chore (a one-off access grant, a toggle, a short follow-up) doesn't need its own block at all — check whether it can just fold into an existing Admin & comms slot instead (see standing preferences).
If it's genuinely ambiguous whether an existing event covers an action, treat it as not covered and schedule the block — err toward the action getting done, and mention the ambiguity in the summary.

## Darryl's standing preferences — do not re-ask these each time

- Working hours: 09:45–19:00 Mon–Thu, 09:45–16:00 Fri.
- **Lunch is opportunistic, not a must-have.** If — and only if — a genuine 30-min gap exists somewhere between **14:00–17:00** on a given day without displacing, shrinking, or overlapping anything else, book a 30-min "Lunch (OOO)" block there. Don't force one in, don't place it outside the 14:00–17:00 window, and don't touch it if the day's already full. **Skip Thursdays entirely** — the recurring "Weekly HI Team Lunch" already covers it.
...
- Meetings/1:1s with other people get a 30-min prep buffer before and a 30-min follow-up buffer after.
  ...
  - Exception: meetings with a DS (Divisional/Department Secretary) don't get this generic buffer at all, before or after — they use the dedicated DS-prep rule below instead.
- **DS meeting prep is its own Focus block, not a buffer** — see "Prepping for DS meetings" below.
- **Chores don't need their own block.** A quick administrative/config action (a one-off access grant, a toggle, a short follow-up email) isn't focus work — fold it into an existing Admin & comms slot instead of creating a dedicated block, and say which slot absorbed it in the summary.
...
## Prepping for DS meetings

DS (Divisional/Department Secretary) meetings are a special case — no generic 30-min buffer, no default 4-hour focus sizing. Instead:

- Schedule a dedicated **"Prep for DS \<name\>"** Focus block, **at least 24 hours before** the actual DS meeting — never same-day, never a same-day buffer.
- Default size: **~3 hours**, adjusted by Duration Log history for that specific DS relationship the same as any other category (see "Sizing blocks").
- One prep block covers everything that meeting needs — a case to present, a written brief, approver feedback, whatever the action calls for. Don't create a second prep block under the action's own wording for the same meeting (see the coverage check above).
- If Darryl mentions he's already done substantial prep for a specific DS meeting this week, treat that meeting's prep as covered — don't schedule a redundant block just because the 24-hour/3-hour defaults would otherwise call for one.
...
5. Create the events directly — any opportunistic lunch block that actually fits, plus all non-conflicting action blocks. See "Creating events."
...
- Lunch blocks (only when one actually fits, see standing preferences): use `eventType: outOfOffice` if the calendar tool supports it; otherwise a plain default-type event titled "Lunch (OOO)" — either way, no attendees, since it's not a meeting.
...
- DS meeting prep is the one exception to the 4-hour default: size it at **~3 hours** and place it **≥24 hours before the DS meeting**, per "Prepping for DS meetings" above — adjust from 3 hours only once there's Duration Log history for that specific DS relationship.
